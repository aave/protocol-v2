...
contract MockBeefyVault is ERC20 {
  ...
  constructor(
    ...
    ERC20(
      string(abi.encodePacked('Moo Scream ', IERC20Detailed(_token).symbol())),
      string(abi.encodePacked('mooScream', IERC20Detailed(_token).symbol()))
    )
  {
    token = ERC20(_token);
  ...
  function withdraw(uint256 _shares) external returns (uint256) {
    ...
    TransferHelper.safeTransfer(address(token), msg.sender, value);
