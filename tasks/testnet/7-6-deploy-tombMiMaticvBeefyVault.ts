...
import { deployTombMiMaticBeefyVault } from '../../helpers/contracts-deployments';
...
task(`testnet:deploy-tomb-mimatic-beefy-vault`, `Deploys the ${CONTRACT_NAME} contract`)
  ...
  .setAction(async ({ verify, pool }, localBRE) => {
    ...
    await waitForTx(
      await priceOracleInstance.setAssetPrice(
        poolConfig.MIMATIC[network],
        poolConfig.Mocks.AllAssetsInitialPrices['MIMATIC']
      )
    );
    await waitForTx(
      await priceOracleInstance.setAssetPrice(
        poolConfig.ReserveAssets[network]['mooTOMB_MIMATIC'],
        poolConfig.Mocks.AllAssetsInitialPrices['mooTOMB_MIMATIC']
      )
    );
