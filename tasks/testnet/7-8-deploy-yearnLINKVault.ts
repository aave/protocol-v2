...
task(`testnet:deploy-yearn-link-vault`, `Deploys the ${CONTRACT_NAME} contract`)
  ...
  .setAction(async ({ verify, pool }, localBRE) => {
    ...
    await waitForTx(
      await priceOracleInstance.setAssetPrice(
        poolConfig.ReserveAssets[network]['yvLINK'],
        poolConfig.Mocks.AllAssetsInitialPrices['yvLINK']
      )
    );
