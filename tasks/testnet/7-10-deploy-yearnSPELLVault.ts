...
task(`testnet:deploy-yearn-spell-vault`, `Deploys the ${CONTRACT_NAME} contract`)
  ...
  .setAction(async ({ verify, pool }, localBRE) => {
    ...
    await waitForTx(
      await priceOracleInstance.setAssetPrice(
        poolConfig.ReserveAssets[network]['yvSPELL'],
        poolConfig.Mocks.AllAssetsInitialPrices['yvSPELL']
      )
    );
