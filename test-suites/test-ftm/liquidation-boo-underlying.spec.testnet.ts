...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  //   const { configurator, yvboo, pool, users, dai, deployer } = testEnv;
  //   const user = users[1];
  //   await configurator.connect(deployer.signer).deactivateReserve(yvboo.address);

  //   await expect(
  //     pool.liquidationCall(yvboo.address, dai.address, user.address, parseEther('7000'), false)
  //   ).to.be.revertedWith('2');

  //   await configurator.connect(deployer.signer).activateReserve(yvboo.address);
  ...
  it('Deposits BOO, borrows DAI', async () => {
    const { dai, users, pool, oracle, yearnBOOVault, deployer, BOO } = testEnv;
    ...
    //user 2 deposits 100 BOO
    const amountBOOtoDeposit = ethers.utils.parseEther('100');
    await BOO.connect(deployer.signer).transfer(borrower.address, amountBOOtoDeposit);

    //approve protocol to access borrower wallet
    await BOO.connect(borrower.signer).approve(yearnBOOVault.address, APPROVAL_AMOUNT_LENDING_POOL);

    await yearnBOOVault
      .connect(borrower.signer)
      .depositCollateral(BOO.address, amountBOOtoDeposit);
  ...
  it('Liquidates the borrow', async () => {
    const { dai, yvboo, BOO, users, pool, oracle, helpersContract, deployer } = testEnv;
    ...
    const ethReserveDataBefore = await helpersContract.getReserveData(yvboo.address);
    ...
    const tx = await pool
      ...
      .liquidationCall(BOO.address, dai.address, borrower.address, amountToLiquidate, false);
    ...
    const ethReserveDataAfter = await helpersContract.getReserveData(yvboo.address);

    const collateralPrice = await oracle.getAssetPrice(yvboo.address);
    const principalPrice = await oracle.getAssetPrice(dai.address);

    const collateralDecimals = (
      await helpersContract.getReserveConfigurationData(yvboo.address)
    ).decimals.toString();
  ...
  it('User 3 deposits 7000 USDC, user 4 100 BOO, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, BOO, helpersContract, yearnBOOVault, deployer, yvboo } = testEnv;
    ...
    //borrower deposits 100 BOO
    const amountBOOtoDeposit = await convertToCurrencyDecimals(BOO.address, '100');
    await BOO.connect(deployer.signer).transfer(borrower.address, amountBOOtoDeposit);
    ...
    await pool
      .connect(liquidator.signer)
      .liquidationCall(BOO.address, usdc.address, borrower.address, amountToLiquidate, false);
