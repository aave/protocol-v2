...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits WETH, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    const { dai, users, pool, oracle, beefyETHVault, deployer, WETH } = testEnv;
    ...
    //prepare WETH to deposit
    await WETH.connect(deployer.signer).transfer(borrower.address, amountWETHtoDeposit);

    //approve protocol to access borrower wallet
    await WETH.connect(borrower.signer).approve(beefyETHVault.address, APPROVAL_AMOUNT_LENDING_POOL);
  ...
  it('Tries to liquidate a different currency than the loan principal', async () => {
    const { pool, users, WETH, usdc } = testEnv;
    const borrower = users[1];
    //user 2 tries to borrow
    await expect(
      pool.liquidationCall(WETH.address, usdc.address, borrower.address, oneEther.toString(), true)
  ...
  it('Tries to liquidate a different collateral than the borrower collateral', async () => {
    const { pool, dai, WFTM, users, mooweth } = testEnv;
    const borrower = users[1];

    await expect(
      pool.liquidationCall(WFTM.address, dai.address, borrower.address, oneEther.toString(), true)
    ).revertedWith(LPCM_COLLATERAL_CANNOT_BE_LIQUIDATED);
  });

  it('Liquidates the borrow', async () => {
    const { pool, dai, WETH, mooweth, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
    ...
    const ethReserveDataBefore = await helpersContract.getReserveData(mooweth.address);
    ...
    const tx = await pool
      ...
      .liquidationCall(WETH.address, dai.address, borrower.address, amountToLiquidate, true);
    ...
    const ethReserveDataAfter = await helpersContract.getReserveData(mooweth.address);
    ...
    const collateralDecimals = (
      await helpersContract.getReserveConfigurationData(mooweth.address)
    ).decimals.toString();
    ...
    expect(
      (await helpersContract.getUserReserveData(mooweth.address, deployer.address))
        .usageAsCollateralEnabled
    ).to.be.true;
  });

  it('User 3 deposits 7000 USDC, user 4 1 WETH, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, WETH, mooweth, helpersContract, beefyETHVault, deployer } = testEnv;
    ...
    await pool
      .connect(deployer.signer)
      .liquidationCall(WETH.address, usdc.address, borrower.address, amountToLiquidate, true);
