...
makeSuite('TombFtmBeefyVault', (testEnv: TestEnv) => {
  ...
  it('deposit TOMB_FTM_LP for collateral', async () => {
    ...
    await TOMB_FTM_LP.connect(deployer.signer).approve(TombFtmBeefyVault.address, amountTombFtmLPtoDeposit);

    await TombFtmBeefyVault.connect(deployer.signer).depositCollateral(TOMB_FTM_LP.address, amountTombFtmLPtoDeposit);
  ...
  it('transferring aMooTOMB_FTM should be success after deposit TOMB_FTM_LP', async () => {
    const { aMooTOMB_FTM, users, deployer } = testEnv;
    await expect(aMooTOMB_FTM.connect(deployer.signer).transfer(users[0].address, await convertToCurrencyDecimals(aMooTOMB_FTM.address, '50'))).to.not.be.reverted;
  });

  it('withdraw from collateral should be failed if user has not enough balance', async () => {
    const { deployer, TombFtmBeefyVault, TOMB_FTM_LP } = testEnv;
    const amountTombFtmLPtoDeposit = await convertToCurrencyDecimals(TOMB_FTM_LP.address, '1500');
    await expect(TombFtmBeefyVault.connect(deployer.signer).withdrawCollateral(TOMB_FTM_LP.address, amountTombFtmLPtoDeposit, 9900, deployer.address))
      .to.be.reverted;
  });
  ...
  it('withdraw from collateral', async () => {
    ...
    await TombFtmBeefyVault.connect(deployer.signer).withdrawCollateral(TOMB_FTM_LP.address, tombFtmLPWithdrawAmount, 9900, deployer.address);
...
makeSuite('TombFtmBeefyVault - use other coin as collateral', (testEnv) => {
  it('Should revert to use any of coin other than TOMB_FTM_LP as collateral', async () => {
    const { usdc, TombFtmBeefyVault, mootomb_ftm, deployer } = testEnv;
    // TODO @bshevchenko: use Error const instead of 82
    await expect(TombFtmBeefyVault.connect(deployer.signer).depositCollateral(mootomb_ftm.address, 1000)).to.be.revertedWith('82');
  });
});
