...
makeSuite('TombMiMaticBeefyVault', (testEnv: TestEnv) => {
  ...
  it('withdraw from collateral', async () => {
    ...
    const mootombftmBalanceOfPool = await mootomb_mimatic.balanceOf(TombMiMaticBeefyVault.address);
...
makeSuite('TombMiMaticBeefyVault - use other coin as collateral', (testEnv) => {
  it('Should revert to use any of coin other than TOMB_MIMATIC_LP as collateral', async () => {
    const { usdc, TombMiMaticBeefyVault, mootomb_mimatic, deployer } = testEnv;
    // TODO @bshevchenko: use Error const instead of 82
    await expect(TombMiMaticBeefyVault.connect(deployer.signer).depositCollateral(mootomb_mimatic.address, 1000)).to.be.revertedWith('82');
  });
});
...
//     const ethers = (DRE as any).ethers;
//     const amountTombMiMaticLPtoDeposit = await convertToCurrencyDecimals(TOMB_MIMATIC_LP.address, '1500');
...
//     const tombMiMaticLPOwnerAddress = '0xbF26ea0DFA032aFeFD43A6C91C50FA6cbe4bA9c6';
...
//     const yieldmootombftmAmount = await convertToCurrencyDecimals(TOMB_MIMATIC_LP.address, '1500');
//     await impersonateAccountsHardhat([mootombftmOwnerAddress]);
//     signer = await ethers.provider.getSigner(mootombftmOwnerAddress);
//     await mootomb_mimatic.connect(signer).transfer(aMooTOMB_MIMATIC.address, yieldmootombftmAmount);
...
//     const tombMiMaticLPOwnerAddress = '0xbF26ea0DFA032aFeFD43A6C91C50FA6cbe4bA9c6';
//     const depositTombMiMaticLP = '1500';
...
//     const yieldmootombftmAmount = await convertToCurrencyDecimals(mootomb_mimatic.address, yieldmootombftm);
//     //Make some test mootomb_mimatic
//     await impersonateAccountsHardhat([mootombftmOwnerAddress]);
//     signer = await ethers.provider.getSigner(mootombftmOwnerAddress);
//     await mootomb_mimatic.connect(signer).transfer(aMooTOMB_MIMATIC.address, yieldmootombftmAmount);

//     expect((await TombMiMaticBeefyVault.getYieldAmount()).gt(parseEther('1499.99'))).to.be.equal(true);
//     expect(await usdc.balanceOf(TombMiMaticBeefyVault.address)).to.be.equal(0);
