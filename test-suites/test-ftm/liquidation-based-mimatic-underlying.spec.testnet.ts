...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  //   const { configurator, moobased_mimatic, pool, users, dai, deployer } = testEnv;
  //   const user = users[1];
  //   await configurator.connect(deployer.signer).deactivateReserve(moobased_mimatic.address);

  //   await expect(
  //     pool.liquidationCall(moobased_mimatic.address, dai.address, user.address, parseEther('7000'), false)
  //   ).to.be.revertedWith('2');

  //   await configurator.connect(deployer.signer).activateReserve(moobased_mimatic.address);
  ...
  it('Deposits BASED_MIMATIC_LP, borrows DAI', async () => {
    ...
    //user 2 deposits 100 BASED_MIMATIC_LP
    const amountTombMiMaticLPtoDeposit = ethers.utils.parseEther('100');
    await BASED_MIMATIC_LP.connect(deployer.signer).transfer(borrower.address, amountTombMiMaticLPtoDeposit);
    ...
    await BasedMiMaticBeefyVault
      .connect(borrower.signer)
      .depositCollateral(BASED_MIMATIC_LP.address, amountTombMiMaticLPtoDeposit);
  ...
  it('Liquidates the borrow', async () => {
    const { dai, moobased_mimatic, BASED_MIMATIC_LP, users, pool, oracle, helpersContract, deployer } = testEnv;
    ...
    const tx = await pool
      ...
      .liquidationCall(BASED_MIMATIC_LP.address, dai.address, borrower.address, amountToLiquidate, false);
    ...
    const collateralPrice = await oracle.getAssetPrice(moobased_mimatic.address);
  ...
  it('User 3 deposits 7000 USDC, user 4 100 BASED_MIMATIC_LP, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, BASED_MIMATIC_LP, helpersContract, BasedMiMaticBeefyVault, deployer, moobased_mimatic } = testEnv;
    ...
    //borrower deposits 100 BOBASED_MIMATIC_LPO
    const amountTombMiMaticLPtoDeposit = await convertToCurrencyDecimals(BASED_MIMATIC_LP.address, '100');
    await BASED_MIMATIC_LP.connect(deployer.signer).transfer(borrower.address, amountTombMiMaticLPtoDeposit);

    //approve protocol to access borrower wallet
    await BASED_MIMATIC_LP.connect(borrower.signer).approve(BasedMiMaticBeefyVault.address, APPROVAL_AMOUNT_LENDING_POOL);

    await BasedMiMaticBeefyVault.connect(borrower.signer).depositCollateral(BASED_MIMATIC_LP.address, amountTombMiMaticLPtoDeposit);
    ...
    await pool
      .connect(liquidator.signer)
      .liquidationCall(BASED_MIMATIC_LP.address, usdc.address, borrower.address, amountToLiquidate, false);
