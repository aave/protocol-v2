...
import { getCollateralAdapter } from '../../helpers/contracts-getters';
...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  //   const { configurator, mooweth, pool, users, dai, deployer } = testEnv;
  //   const user = users[1];
  //   await configurator.connect(deployer.signer).deactivateReserve(mooweth.address);

  //   await expect(
  //     pool.liquidationCall(mooweth.address, dai.address, user.address, parseEther('7000'), false)
  //   ).to.be.revertedWith('2');

  //   await configurator.connect(deployer.signer).activateReserve(mooweth.address);
  ...
  it('Deposits WETH, borrows DAI', async () => {
    ...
    const amountWETHtoDeposit = ethers.utils.parseEther('1');
    ...
    await beefyETHVault
      ...
      .depositCollateral(WETH.address, amountWETHtoDeposit);
  ...
  it('Liquidates the borrow', async () => {
    const { dai, mooweth, WETH, users, pool, oracle, helpersContract, deployer } = testEnv;
    ...
    const collateralPrice = await oracle.getAssetPrice(mooweth.address);
  ...
  it('User3 deposits 7000 USDC, user4 1 WETH, user4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, WETH, helpersContract, beefyETHVault, deployer, mooweth } = testEnv;
