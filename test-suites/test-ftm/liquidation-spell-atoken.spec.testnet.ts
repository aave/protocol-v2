...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits SPELL, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    const { dai, users, pool, oracle, yearnSPELLVault, deployer, SPELL, yvspell, aYVSPELL } = testEnv;
    ...
    const amountSPELLtoDeposit = await convertToCurrencyDecimals(SPELL.address, '100');

    //prepare SPELL to deposit
    ...
    await expect(
      pool.liquidationCall(SPELL.address, dai.address, borrower.address, 1, true)
  ...
  it('Tries to liquidate a different currency than the loan principal', async () => {
    const { pool, users, SPELL, usdc } = testEnv;
    const borrower = users[1];
    //user 2 tries to borrow
    await expect(
      pool.liquidationCall(SPELL.address, usdc.address, borrower.address, oneEther.toString(), true)
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, SPELL, yvspell, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
    ...
    const tx = await pool
      .connect(deployer.signer)
      .liquidationCall(SPELL.address, dai.address, borrower.address, amountToLiquidate, true);
    ...
    const collateralPrice = (await oracle.getAssetPrice(SPELL.address)).toString();
    ...
    expect(
      (await helpersContract.getUserReserveData(yvspell.address, deployer.address))
        .usageAsCollateralEnabled
    ).to.be.true;
  });

  it('User3 deposits 7000 USDC, user4 3000 SPELL, user4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, SPELL, yvspell, helpersContract, yearnSPELLVault, deployer } = testEnv;
    ...
    await pool
      .connect(deployer.signer)
      .liquidationCall(SPELL.address, usdc.address, borrower.address, amountToLiquidate, true);
