...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  before(async () => {
    const { configurator, yvcrv, deployer } = testEnv;
    await configurator
      .connect(deployer.signer)
      .configureReserveAsCollateral(yvcrv.address, '7000', '7500', '10750');
  });

  it('Deposits CRV, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    const { dai, users, pool, oracle, yearnCRVVault, deployer, CRV, yvcrv, aYVCRV } = testEnv;
    ...
    await expect(
      pool.liquidationCall(CRV.address, dai.address, borrower.address, 1, true)
  ...
  it('Tries to liquidate a different currency than the loan principal', async () => {
    const { pool, users, CRV, usdc } = testEnv;
    const borrower = users[1];
    //user 2 tries to borrow
    await expect(
      pool.liquidationCall(CRV.address, usdc.address, borrower.address, oneEther.toString(), true)
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, CRV, yvcrv, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
    ...
    const tx = await pool
      .connect(deployer.signer)
      .liquidationCall(CRV.address, dai.address, borrower.address, amountToLiquidate, true);
    ...
    const collateralPrice = (await oracle.getAssetPrice(CRV.address)).toString();
    ...
    expect(
      (await helpersContract.getUserReserveData(yvcrv.address, deployer.address))
        .usageAsCollateralEnabled
    ).to.be.true;
  });

  it('User3 deposits 7000 USDC, user4 3000 CRV, user4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, CRV, yvcrv, helpersContract, yearnCRVVault, deployer } = testEnv;
    ...
    //user4 deposits 3000 CRV
    const amountCRVtoDeposit = await convertToCurrencyDecimals(CRV.address, '3000');
    await CRV.connect(deployer.signer).transfer(borrower.address, amountCRVtoDeposit);
    ...
    await pool
      .connect(deployer.signer)
      .liquidationCall(CRV.address, usdc.address, borrower.address, amountToLiquidate, true);
