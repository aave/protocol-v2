...
makeSuite('yearnWBTCVault', (testEnv: TestEnv) => {
  ...
  it('deposit WBTC for collateral', async () => {
    ...
    await yearnWBTCVault.connect(deployer.signer).depositCollateral(WBTC.address, amountWBTCtoDeposit);
  ...
  it('transferring aYVWBTC should be success after deposit WBTC', async () => {
    const { aYVWBTC, users, deployer, WBTC } = testEnv;
    await expect(aYVWBTC.connect(deployer.signer).transfer(users[0].address, await convertToCurrencyDecimals(aYVWBTC.address, '0.0009'))).to.not.be.reverted;
  });
  ...
  it('withdraw from collateral should be failed if user has not enough balance', async () => {
    ...
    await expect(yearnWBTCVault.connect(deployer.signer).withdrawCollateral(WBTC.address, amountWBTCtoDeposit, 9900, deployer.address))
      .to.be.reverted;
  });
  ...
  it('withdraw from collateral', async () => {
    ...
    await yearnWBTCVault.connect(deployer.signer).withdrawCollateral(WBTC.address, wbtcWithdrawAmount, 9900, deployer.address);
...
makeSuite('yearnWBTCVault - use other coin as collateral', (testEnv) => {
  it('Should revert to use any of coin other than WBTC as collateral', async () => {
    const { usdc, yearnWBTCVault, deployer } = testEnv;
    // TODO @bshevchenko: use Error const instead of 82
    await expect(yearnWBTCVault.connect(deployer.signer).depositCollateral(usdc.address, 1000)).to.be.revertedWith('82');
  });
});
