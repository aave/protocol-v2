...
import { getLendingPoolCollateralManager, getCollateralAdapter } from '../../helpers/contracts-getters';
...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  before(async () => {
    const { configurator, yvcrv, deployer, CRV } = testEnv;
    await configurator
      .connect(deployer.signer)
      .configureReserveAsCollateral(yvcrv.address, '7000', '7500', '10750');
    
    const collateralManager = await getLendingPoolCollateralManager();
    const collateralAdapter = await getCollateralAdapter();
    const vault = await collateralAdapter.getAcceptableVault(CRV.address);
  ...
  //   const { configurator, yvcrv, pool, users, dai, deployer } = testEnv;
  //   const user = users[1];
  //   await configurator.connect(deployer.signer).deactivateReserve(yvcrv.address);

  //   await expect(
  //     pool.liquidationCall(yvcrv.address, dai.address, user.address, parseEther('7000'), false)
  //   ).to.be.revertedWith('2');

  //   await configurator.connect(deployer.signer).activateReserve(yvcrv.address);
  ...
  it('Deposits CRV, borrows DAI', async () => {
    const { dai, users, pool, oracle, yearnCRVVault, deployer, CRV, yvcrv } = testEnv;
    ...
    //user 2 deposits 100 CRV
    const amountCRVtoDeposit = ethers.utils.parseEther('100');
    await CRV.connect(deployer.signer).transfer(borrower.address, amountCRVtoDeposit);

    //approve protocol to access borrower wallet
    await CRV.connect(borrower.signer).approve(yearnCRVVault.address, APPROVAL_AMOUNT_LENDING_POOL);

    await yearnCRVVault
      .connect(borrower.signer)
      .depositCollateral(CRV.address, amountCRVtoDeposit);
  ...
  it('Liquidates the borrow', async () => {
    const { dai, yvcrv, CRV, users, pool, oracle, helpersContract, deployer, yearnCRVVault } = testEnv;
    ...
    const ethReserveDataBefore = await helpersContract.getReserveData(yvcrv.address);
    ...
    const tx = await pool
      ...
      .liquidationCall(CRV.address, dai.address, borrower.address, amountToLiquidate, false);
    ...
    const ethReserveDataAfter = await helpersContract.getReserveData(yvcrv.address);

    const collateralPrice = await oracle.getAssetPrice(yvcrv.address);
    const principalPrice = await oracle.getAssetPrice(dai.address);

    const collateralDecimals = (
      await helpersContract.getReserveConfigurationData(yvcrv.address)
    ).decimals.toString();
  ...
  it('User 3 deposits 7000 USDC, user 4 100 CRV, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, CRV, helpersContract, yearnCRVVault, deployer, yvcrv } = testEnv;
    ...
    await pool
      .connect(liquidator.signer)
      .liquidationCall(CRV.address, usdc.address, borrower.address, amountToLiquidate, false);
