...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits BOO, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    ...
    const amountBOOtoDeposit = await convertToCurrencyDecimals(BOO.address, '1');

    //prepare BOO to deposit
    ...
    await expect(
      pool.liquidationCall(BOO.address, dai.address, borrower.address, 1, true)
  ...
  it('Tries to liquidate a different currency than the loan principal', async () => {
    const { pool, users, BOO, usdc } = testEnv;
    const borrower = users[1];
    //user 2 tries to borrow
    await expect(
      pool.liquidationCall(BOO.address, usdc.address, borrower.address, oneEther.toString(), true)
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, BOO, yvboo, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
    ...
    const tx = await pool
      ...
      .liquidationCall(BOO.address, dai.address, borrower.address, amountToLiquidate, true);
    ...
    const collateralPrice = (await oracle.getAssetPrice(BOO.address)).toString();
    ...
    expect(
      (await helpersContract.getUserReserveData(yvboo.address, deployer.address))
        .usageAsCollateralEnabled
    ).to.be.true;
  });

  it('User 3 deposits 7000 USDC, user 4 1 BOO, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, BOO, yvboo, helpersContract, yearnBOOVault, deployer } = testEnv;
    ...
    await pool
      .connect(deployer.signer)
      .liquidationCall(BOO.address, usdc.address, borrower.address, amountToLiquidate, true);
