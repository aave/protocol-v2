...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  //   const { configurator, mootomb_mimatic, pool, users, dai, deployer } = testEnv;
  //   const user = users[1];
  //   await configurator.connect(deployer.signer).deactivateReserve(mootomb_mimatic.address);

  //   await expect(
  //     pool.liquidationCall(mootomb_mimatic.address, dai.address, user.address, parseEther('7000'), false)
  //   ).to.be.revertedWith('2');

  //   await configurator.connect(deployer.signer).activateReserve(mootomb_mimatic.address);
  ...
  it('Deposits TOMB_MIMATIC_LP, borrows DAI', async () => {
    ...
    //user 2 deposits 100 TOMB_MIMATIC_LP
    const amountTombMiMaticLPtoDeposit = ethers.utils.parseEther('100');
    await TOMB_MIMATIC_LP.connect(deployer.signer).transfer(borrower.address, amountTombMiMaticLPtoDeposit);

    //approve protocol to access borrower wallet
    await TOMB_MIMATIC_LP.connect(borrower.signer).approve(TombMiMaticBeefyVault.address, APPROVAL_AMOUNT_LENDING_POOL);

    await TombMiMaticBeefyVault
      .connect(borrower.signer)
      .depositCollateral(TOMB_MIMATIC_LP.address, amountTombMiMaticLPtoDeposit);
  ...
  it('Liquidates the borrow', async () => {
    const { dai, mootomb_mimatic, TOMB_MIMATIC_LP, users, pool, oracle, helpersContract, deployer } = testEnv;
    ...
    const tx = await pool
      ...
      .liquidationCall(TOMB_MIMATIC_LP.address, dai.address, borrower.address, amountToLiquidate, false);
    ...
    const collateralPrice = await oracle.getAssetPrice(mootomb_mimatic.address);
  ...
  it('User 3 deposits 7000 USDC, user 4 100 TOMB_MIMATIC_LP, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, TOMB_MIMATIC_LP, helpersContract, TombMiMaticBeefyVault, deployer, mootomb_mimatic } = testEnv;
    ...
    //borrower deposits 100 BOTOMB_MIMATIC_LPO
    const amountTombMiMaticLPtoDeposit = await convertToCurrencyDecimals(TOMB_MIMATIC_LP.address, '100');
    await TOMB_MIMATIC_LP.connect(deployer.signer).transfer(borrower.address, amountTombMiMaticLPtoDeposit);
    ...
    await pool
      .connect(liquidator.signer)
      .liquidationCall(TOMB_MIMATIC_LP.address, usdc.address, borrower.address, amountToLiquidate, false);
