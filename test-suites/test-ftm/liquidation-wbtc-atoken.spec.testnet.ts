...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits WBTC, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    const { dai, users, pool, oracle, yearnWBTCVault, deployer, WBTC, yvwbtc } = testEnv;
    ...
    await expect(
      pool.liquidationCall(WBTC.address, dai.address, borrower.address, 1, true)
  ...
  it('Tries to liquidate a different currency than the loan principal', async () => {
    const { pool, users, WBTC, usdc } = testEnv;
    const borrower = users[1];
    //user 2 tries to borrow
    await expect(
      pool.liquidationCall(WBTC.address, usdc.address, borrower.address, oneEther.toString(), true)
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, WBTC, yvwbtc, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
    ...
    const tx = await pool
      ...
      .liquidationCall(WBTC.address, dai.address, borrower.address, amountToLiquidate, true);
    ...
    const collateralPrice = (await oracle.getAssetPrice(WBTC.address)).toString();
    ...
    expect(
      (await helpersContract.getUserReserveData(yvwbtc.address, deployer.address))
        .usageAsCollateralEnabled
    ).to.be.true;
  });

  it('User 3 deposits 7000 USDC, user 4 0.1 WBTC, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, WBTC, yvwbtc, helpersContract, yearnWBTCVault, deployer } = testEnv;
    ...
    await pool
      .connect(deployer.signer)
      .liquidationCall(WBTC.address, usdc.address, borrower.address, amountToLiquidate, true);
