...
makeSuite('yearnBOOVault', (testEnv: TestEnv) => {
  it('failed deposit for collateral without BOO', async () => {
    const { yearnBOOVault } = testEnv;
    await expect(yearnBOOVault.depositCollateral(ZERO_ADDRESS, 0)).to.be.reverted;
  });
  ...
  it('deposit BOO for collateral', async () => {
    ...
    let signer = await ethers.provider.getSigner(booOwnerAddress);
    await BOO.connect(signer).transfer(deployer.address, amountBOOtoDeposit);
    
    await BOO.approve(yearnBOOVault.address, amountBOOtoDeposit);

    await yearnBOOVault.depositCollateral(BOO.address, amountBOOtoDeposit);
  ...
  it('transferring aYVBOO should be success after deposit BOO', async () => {
    const { aYVBOO, users } = testEnv;
    await expect(aYVBOO.transfer(users[0].address, await convertToCurrencyDecimals(aYVBOO.address, '10'))).to.not.be.reverted;
  });
