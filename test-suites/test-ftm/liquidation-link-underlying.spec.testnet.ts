...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  //   const { configurator, yvlink, pool, users, dai, deployer } = testEnv;
  //   const user = users[1];
  //   await configurator.connect(deployer.signer).deactivateReserve(yvlink.address);

  //   await expect(
  //     pool.liquidationCall(yvlink.address, dai.address, user.address, parseEther('7000'), false)
  //   ).to.be.revertedWith('2');

  //   await configurator.connect(deployer.signer).activateReserve(yvlink.address);
  ...
  it('Deposits LINK, borrows DAI', async () => {
    ...
    //user 2 deposits 100 LINK
    const amountLINKtoDeposit = ethers.utils.parseEther('100');
    ...
    await yearnLINKVault
      .connect(borrower.signer)
      .depositCollateral(LINK.address, amountLINKtoDeposit);
  ...
  it('Liquidates the borrow', async () => {
    const { dai, yvlink, LINK, users, pool, oracle, helpersContract, deployer } = testEnv;
    ...
    const tx = await pool
      ...
      .liquidationCall(LINK.address, dai.address, borrower.address, amountToLiquidate, false);
    ...
    const collateralPrice = await oracle.getAssetPrice(yvlink.address);
  ...
  it('User 3 deposits 7000 USDC, user 4 100 LINK, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, LINK, helpersContract, yearnLINKVault, deployer, yvlink } = testEnv;
    ...
    await pool
      .connect(liquidator.signer)
      .liquidationCall(LINK.address, usdc.address, borrower.address, amountToLiquidate, false);
