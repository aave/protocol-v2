...
makeSuite('AddressesProviderRegistry', (testEnv: TestEnv) => {
  ...
  it('tries to register an addresses provider with id 0', async () => {
    const { users, registry, registryOwnerSigner } = testEnv;
    const { LPAPR_INVALID_ADDRESSES_PROVIDER_ID } = ProtocolErrors;

    await expect(
      registry.connect(registryOwnerSigner).registerAddressesProvider(users[2].address, '0')
  ...
  it('Registers a new mock addresses provider', async () => {
    ...
    await registry.connect(registryOwnerSigner).registerAddressesProvider(users[1].address, '2');
  ...
  it('Removes the mock addresses provider', async () => {
    const { users, registry, addressesProvider, registryOwnerSigner } = testEnv;
    ...
    await registry.connect(registryOwnerSigner).unregisterAddressesProvider(users[1].address);
  ...
  it('Tries to remove a unregistered addressesProvider', async () => {
    ...
    await expect(
      registry.connect(registryOwnerSigner).unregisterAddressesProvider(users[2].address)
  ...
  it('Tries to add an already added addressesProvider with a different id. Should overwrite the previous id', async () => {
    ...
    await registry
      ...
      .registerAddressesProvider(addressesProvider.address, '2');
