...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits WETH, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    const { dai, users, pool, oracle, yearnWETHVault, deployer, WETH, yvweth } = testEnv;
    ...
    await WETH.connect(borrower.signer).approve(yearnWETHVault.address, APPROVAL_AMOUNT_LENDING_POOL);
  ...
  it('Tries to liquidate a different collateral than the borrower collateral', async () => {
    const { pool, dai, WFTM, users } = testEnv;
    const borrower = users[1];
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, WETH, yvweth, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
    ...
    const ethReserveDataBefore = await helpersContract.getReserveData(yvweth.address);
    ...
    const ethReserveDataAfter = await helpersContract.getReserveData(yvweth.address);
    ...
    const collateralDecimals = (
      await helpersContract.getReserveConfigurationData(yvweth.address)
    ).decimals.toString();
    ...
    expect(
      (await helpersContract.getUserReserveData(yvweth.address, deployer.address))
        .usageAsCollateralEnabled
    ).to.be.true;
  });

  it('User 3 deposits 7000 USDC, user 4 1 WETH, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, WETH, yvweth, helpersContract, yearnWETHVault, deployer } = testEnv;
