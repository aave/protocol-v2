...
makeSuite('yearnWETHVault', (testEnv: TestEnv) => {
  ...
  it('deposit WETH for collateral', async () => {
    ...
    await WETH.connect(deployer.signer).approve(yearnWETHVault.address, amountWETHtoDeposit);

    await yearnWETHVault.connect(deployer.signer).depositCollateral(WETH.address, amountWETHtoDeposit);
  ...
  it('transferring aYVWETH should be success after deposit WETH', async () => {
    const { aYVWETH, users, deployer } = testEnv;
    await expect(aYVWETH.connect(deployer.signer).transfer(users[0].address, parseEther('0.09'))).to.not.be.reverted;
  });

  it('withdraw from collateral should be failed if user has not enough balance', async () => {
    const { deployer, yearnWETHVault, WETH } = testEnv;
    await expect(yearnWETHVault.connect(deployer.signer).withdrawCollateral(WETH.address, amountWETHtoDeposit.add(beforeBalance), 9900, deployer.address))
      .to.be.reverted;
  });
  ...
  it('withdraw from collateral', async () => {
    ...
    await yearnWETHVault.connect(deployer.signer).withdrawCollateral(WETH.address, parseEther('0.9'), 9900, deployer.address);
...
makeSuite('yearnWETHVault - use other coin as collateral', (testEnv) => {
  it('Should revert to use any of coin other than WETH as collateral', async () => {
    const { usdc, yearnWETHVault, deployer } = testEnv;
    // TODO @bshevchenko: use Error const instead of 82
    await expect(yearnWETHVault.connect(deployer.signer).depositCollateral(usdc.address, 1000)).to.be.revertedWith('82');
  });
});
