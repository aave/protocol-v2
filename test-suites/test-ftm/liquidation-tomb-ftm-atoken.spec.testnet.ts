...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits TOMB_FTM_LP, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    ...
    const amountTombFtmLPtoDeposit = await convertToCurrencyDecimals(TOMB_FTM_LP.address, '1');

    //prepare TOMB_FTM_LP to deposit
    ...
    await expect(
      pool.liquidationCall(TOMB_FTM_LP.address, dai.address, borrower.address, 1, true)
  ...
  it('Tries to liquidate a different currency than the loan principal', async () => {
    const { pool, users, TOMB_FTM_LP, usdc } = testEnv;
    const borrower = users[1];
    //user 2 tries to borrow
    await expect(
      pool.liquidationCall(TOMB_FTM_LP.address, usdc.address, borrower.address, oneEther.toString(), true)
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, TOMB_FTM_LP, mootomb_ftm, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
    ...
    const tx = await pool
      ...
      .liquidationCall(TOMB_FTM_LP.address, dai.address, borrower.address, amountToLiquidate, true);
    ...
    const collateralPrice = (await oracle.getAssetPrice(TOMB_FTM_LP.address)).toString();
    ...
    expect(
      (await helpersContract.getUserReserveData(mootomb_ftm.address, deployer.address))
        .usageAsCollateralEnabled
    ).to.be.true;
  });

  it('User 3 deposits 7000 USDC, user 4 1 TOMB_FTM_LP, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, TOMB_FTM_LP, mootomb_ftm, helpersContract, TombFtmBeefyVault, deployer } = testEnv;
    ...
    await pool
      .connect(deployer.signer)
      .liquidationCall(TOMB_FTM_LP.address, usdc.address, borrower.address, amountToLiquidate, true);
