...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits fBEETS, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    ...
    await expect(
      pool.liquidationCall(fBEETS.address, dai.address, borrower.address, 1, true)
  ...
  it('Tries to liquidate a different currency than the loan principal', async () => {
    const { pool, users, fBEETS, usdc } = testEnv;
    const borrower = users[1];
    //user 2 tries to borrow
    await expect(
      pool.liquidationCall(fBEETS.address, usdc.address, borrower.address, oneEther.toString(), true)
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, fBEETS, yvfbeets, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
    ...
    const tx = await pool
      ...
      .liquidationCall(fBEETS.address, dai.address, borrower.address, amountToLiquidate, true);
    ...
    const collateralPrice = (await oracle.getAssetPrice(fBEETS.address)).toString();
    ...
    expect(
      (await helpersContract.getUserReserveData(yvfbeets.address, deployer.address))
        .usageAsCollateralEnabled
    ).to.be.true;
  });

  it('User3 deposits 7000 USDC, user4 1 fBEETS, user4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, fBEETS, yvfbeets, helpersContract, yearnFBEETSVault, deployer } = testEnv;
    ...
    //user4 deposits 1 fBEETS
    const amountfBEETStoDeposit = await convertToCurrencyDecimals(fBEETS.address, '1');
    await fBEETS.connect(deployer.signer).transfer(borrower.address, amountfBEETStoDeposit);
    ...
    await pool
      .connect(deployer.signer)
      .liquidationCall(fBEETS.address, usdc.address, borrower.address, amountToLiquidate, true);
