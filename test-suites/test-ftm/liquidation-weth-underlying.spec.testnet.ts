...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  //   const { configurator, yvweth, pool, users, dai, deployer } = testEnv;
  //   const user = users[1];
  //   await configurator.connect(deployer.signer).deactivateReserve(yvweth.address);

  //   await expect(
  //     pool.liquidationCall(yvweth.address, dai.address, user.address, parseEther('7000'), false)
  //   ).to.be.revertedWith('2');

  //   await configurator.connect(deployer.signer).activateReserve(yvweth.address);
  ...
  it('Deposits WETH, borrows DAI', async () => {
    const { dai, users, pool, oracle, yearnWETHVault, deployer, WETH } = testEnv;
  ...
  it('Liquidates the borrow', async () => {
    const { dai, yvweth, WETH, users, pool, oracle, helpersContract, deployer } = testEnv;
    ...
    const collateralPrice = await oracle.getAssetPrice(yvweth.address);
  ...
  it('User 3 deposits 7000 USDC, user 4 1 WETH, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, WETH, helpersContract, yearnWETHVault, deployer, yvweth } = testEnv;
