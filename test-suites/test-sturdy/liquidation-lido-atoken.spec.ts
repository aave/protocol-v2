...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Drop the health factor below 1', async () => {
    const { dai, users, pool, oracle, lido } = testEnv;
    const borrower = users[1];

    const configurator = await getLendingPoolConfiguratorProxy();
    await configurator.configureReserveAsCollateral(lido.address, '5000', '5500', '10500');
  ...
  it('User 3 deposits 7000 USDC, user 4 1 stETH, user 4 borrows - drops HF, liquidates the borrow', async () => {
    ...
    const configurator = await getLendingPoolConfiguratorProxy();
    await configurator.configureReserveAsCollateral(lido.address, '3000', '3500', '10500');
